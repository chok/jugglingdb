...
module.exports = function init(root, options) {
    ...
    if (typeof railway.orm._schemas == 'undefined') {
        railway.orm._schemas = [];
    }

    var relativeConfFile = options.config || '/config/database';
    var confFile = (root || app.root) + relativeConfFile;
    var appConf = options.database || app.get('database');
    var config = appConf || {};
    if (typeof options.global == 'undefined' || options.global === true) {
        railway.orm.config = appConf;
    }
    ...
    if (!railway.parent) {
        if (!appConf) {
            try {
                ...
                console.log('Could not load '+relativeConfFile+'.{js|json|yml}');
                throw e;
            }
        }
    ...
    if (schema.waitForConnect) {
        schema.on('connected', function() {
            loadSchema(schema, railway, app, models, options);
        });
    } else {
        loadSchema(schema, railway, app, models, options);
    }
...
function loadSchema(schema, railway, app, models, options) {
    ...
    // run schema first
    var schemaFile = options.schema || '/db/schema';
    schemaFile = (root || app.root) + schemaFile + '.';
    ...
    count = railway.orm._schemas.length;
    callback = function() {
        count--;
        if (count <= 0) {
            if (typeof options.callback == 'function') {
                options.callback(null, schema);
            }
        }
    };
